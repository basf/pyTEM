...
def perform_tilt_series(microscope, acquisition_properties, shifts):
    """
    ...
    for i in range(len(acquisition_properties.alphas)):
...
class TiltingThread (threading.Thread):
    ...
    def __init__(self, microscope, acquisition_properties):
...
class TakingImagesThread (threading.Thread):
    ...
    def run(self):
        print("Starting taking images at " + str(time.ctime(time.time())))

        self.microscope.set_stage_position_alpha(self, alpha=self.acquisition_properties.alpha_arr[-1],
                                                 speed=self.acquisition_properties.tilt_speed, movement_type="go")

        print("Stopping taking images at " + str(time.ctime(time.time())))
...
if __name__ == "__main__":
    ...
    perform_tilt_series(microscope=scope, acquisition_properties=aqc_props, shifts=shifts_)
