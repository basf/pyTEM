...
def get_correctional_factors() -> Union[np.ndarray, None]:
    ...
    analyzer_angle_label = ttk.Label(root, text="Analyzer Angle, \u03C6 [deg]", wraplength=window_width, font=font,
                                     justify='center')
    analyzer_angle_label.grid(row=4, column=2, padx=5, pady=5)
    polarizer_angle_label = ttk.Label(root, text="Polarizer Angle, \u03B8 [deg]", wraplength=window_width, font=font,
                                      justify='center')
    polarizer_angle_label.grid(row=6, column=0, padx=5, pady=5)
    ...
    for i, label in enumerate(vertical_deg_labels):
        label.grid(row=6 + i, column=1, padx=5, pady=5)

    # Preallocate a matrix for the correctional factors, initially, they are all just 1.0.
    correctional_factors = np.full(shape=(4, 4), fill_value=1.0, dtype='float64')
    ...
    observed_intensity_entry_boxes = np.asarray(
        [[ttk.Entry(root, textvariable=correctional_factors[i][j]) for j in range(4)] for i in range(4)]
    )

    # Add labels where we show the correctional factors in blue.
    ...
    for (row, col), correctional_factor_label in np.ndenumerate(correctional_factor_labels):
        ...
        correctional_factor_label.grid(row=row_offset + row, column=col_offset + col, padx=5, pady=5)
    ...
    continue_button.grid(row=10, column=0, columnspan=6, padx=5, pady=5)
    ...
    exit_button.grid(row=11, column=0, columnspan=6, padx=5, pady=5)
